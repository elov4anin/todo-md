...
Валидатор `todo-md validate` проверяет, что агент в полях `author`/`assignee` входит в этот список. Этот список можно расширить собственными агентами и ролями через конфиг `.todo-md.json` — см. [CONFIG.md](./CONFIG.md).
