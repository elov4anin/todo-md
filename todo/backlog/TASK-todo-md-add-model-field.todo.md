...
*   **Где делается:** docs/todo-md/templates/, docs/todo-md/reference/, src/domain.ts, src/validator.ts, src/export.ts, src/dashboard.ts
...
- [ ] Константа `MODELS` с массивом идентификаторов моделей в `src/domain.ts`.
- [ ] Валидация поля model в `src/validator.ts` (опционально, проверка по списку).
- [ ] Экспорт поля model в `extractRecord()`.
...
- [ ] Поддержка кастомного списка моделей через `.todo-md.json` (config['models']).
...
3. [ ] Добавить константу `MODELS` в `src/domain.ts`.
4. [ ] Добавить правило `validateModel()` в `src/validator.ts`.
5. [ ] Вызвать `validateModel()` при проверке front matter.
6. [ ] Добавить model в `extractRecord()`.
7. [ ] Добавить панель и chart для моделей в dashboard template/browser script.
...
npx todo-md dashboard todo/ -o /tmp/dashboard.html
```
