...
export function initProject(target, options = {}) {
    ...
    if (!existsSync(targetDir))
        throw new TodoMdError(`target directory does not exist: ${target}`);
    ...
    if (!existsSync(sourceDocs))
        throw new TodoMdError(`package docs/ not found at ${sourceDocs}`);
    const result = { lines: [], copied: 0, updated: 0, skipped: 0 };
    if (options.force)
        result.lines.push("  ⚠  --force: existing files will be overwritten.");
    for (const directory of ["todo", "todo/backlog", "todo/done", "todo/cancelled"]) {
        ...
        if (!existsSync(absolute)) {
            ...
            result.lines.push(`  Exists   ${absolute}/`);
    ...
    result.lines.push("", `Done. ${result.copied} copied, ${result.updated} updated, ${result.skipped} skipped.`, "", "Next steps:", `  1. Add this line to your project's AGENTS.md:`, `     * Регламент работы с задачами: [\`${agentsPath}\`](${agentsPath}).`, "  2. Create tasks: npx todo-md create TASK-<category>-<name> --type=<type> --author=\"<роль> (<агент>)\"", "  3. Validate tasks: npx todo-md validate");
    return result;
}
function findPackageRoot(start) {
    let directory = resolve(start);
    while (true) {
        if (existsSync(resolve(directory, "package.json")) && existsSync(resolve(directory, "docs/todo-md")))
            return directory;
...
function copyTree(source, destination, displayRoot, force, result) {
    ...
    for (const entry of readdirSync(source, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            copyTree(from, to, display, force, result);
        else if (entry.isFile())
            copyOne(from, to, display, force, result);
    }
}
function copyOne(source, destination, display, force, result) {
...
function writeGenerated(destination, content, display, force, result) {
    const existed = existsSync(destination);
    if (existed && !force) {
        result.lines.push(`  Skip     ${display} (already exists)`);
        result.skipped += 1;
...
function updateGitignore(file, entry, result) {
