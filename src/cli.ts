...
import { type TodoMdConfig, TodoMdError, type ExportRecord } from "./domain.js";
...
export function main(argv: string[], io: CliIo = PROCESS_IO): number {
...
function parseArgs(args: string[], valueOptions: Set<string> = new Set()): ParsedArgs {
  const parsed: ParsedArgs = { values: [], options: {}, flags: new Set() };
  for (let index = 0; index < args.length; index += 1) {
    const arg = args[index] ?? "";
    if (arg.startsWith("--")) {
      const body = arg.slice(2);
      const equal = body.indexOf("=");
      if (equal >= 0) parsed.options[body.slice(0, equal)] = body.slice(equal + 1);
      else if (valueOptions.has(body) && args[index + 1] !== undefined) {
        parsed.options[body] = args[index + 1] ?? "";
        index += 1;
      } else parsed.flags.add(body);
...
function commandInit(args: string[], io: CliIo): number {
  const parsed = parseArgs(args, new Set(["docs-path", "agents-path"]));
  if (hasHelp(parsed)) return showHelp(io, INIT_HELP);
  rejectUnknown(parsed, new Set(["force"]), new Set(["docs-path", "agents-path"]));
  const target = parsed.values[0] ?? io.cwd();
  if (parsed.values.length > 1) throw new TodoMdError(`unexpected argument: ${parsed.values[1]}`);
  const options: { force?: boolean; docsPath?: string; agentsPath?: string } = {};
  if (parsed.flags.has("force")) options.force = true;
  if (parsed.options["docs-path"] !== undefined) options.docsPath = parsed.options["docs-path"];
  if (parsed.options["agents-path"] !== undefined) options.agentsPath = parsed.options["agents-path"];
  const result = initProject(target, options);
  io.stdout(`${result.lines.join("\n")}\n`);
  return 0;
}

function commandCreate(args: string[], io: CliIo): number {
  const parsed = parseArgs(args, CREATE_OPTIONS);
  if (hasHelp(parsed)) return showHelp(io, CREATE_HELP);
  rejectUnknown(parsed, new Set(), CREATE_OPTIONS);
  const id = parsed.values[0];
  if (!id) return usageError(io, "Error: task/epic ID required.\n", CREATE_HELP);
  const author = parsed.options.author;
  if (!author?.trim()) return usageError(io, "Error: --author=<role> is required.\n", CREATE_HELP);
...
function commandTransition(args: string[], io: CliIo, status: string, verb: string): number {
  const parsed = parseArgs(args, new Set(["assignee", "root"]));
  if (hasHelp(parsed)) return showHelp(io, transitionHelp(verb, status));
  rejectUnknown(parsed, new Set(), new Set(["assignee", "root"]));
  const id = parsed.values[0];
  if (!id) return usageError(io, "Error: task ID required.\n", transitionHelp(verb, status));
...
function commandSet(args: string[], io: CliIo): number {
  const parsed = parseArgs(args, new Set(["root"]));
  if (hasHelp(parsed)) return showHelp(io, SET_HELP);
  ...
  for (const assignment of parsed.values.slice(1)) {
    ...
    if (equal <= 0) return usageError(io,
      `Error: arguments after <ID> must be <field>=<value>, got: ${assignment}\n`, SET_HELP);
...
function commandValidate(args: string[], io: CliIo): number {
  const parsed = parseArgs(args, new Set(["config"]));
  if (hasHelp(parsed)) return showHelp(io, VALIDATE_HELP);
  ...
  let config: TodoMdConfig;
  if (parsed.options.config) config = loadConfigFile(parsed.options.config);
  else {
    let configRoot: string | null = null;
    try { configRoot = resolveRoot(io.cwd()); } catch { /* standalone target */ }
    config = configRoot ? loadConfig(configRoot) : { roles: [], agents: [], strict: false };
  }
  if (parsed.flags.has("strict")) config = { ...config, strict: true };
  ...
  for (const file of files) {
    ...
    if (result.errors.length === 0 && result.warnings.length === 0) io.stdout(`✓ ${relative}\n`);
    else {
      io.stdout(`${result.errors.length === 0 ? "!" : "✗"} ${relative}\n`);
      for (const error of result.errors) io.stdout(`  error: ${error}\n`);
...
function commandExport(args: string[], io: CliIo): number {
  ...
  if (parsed.flags.has("o")) {
    ...
    if (output) parsed.options.output = output;
    parsed.values = parsed.values.filter((value) => value !== args[index + 1]);
    parsed.flags.delete("o");
  }
  if (hasHelp(parsed)) return showHelp(io, EXPORT_HELP);
  ...
  if (parsed.options.output) writeFileSync(parsed.options.output, output);
  else io.stdout(output);
  return 0;
}

function commandDashboard(args: string[], io: CliIo): number {
  const normalized = normalizeShortOutput(args);
  const parsed = parseArgs(normalized, new Set(["output", "title", "base"]));
  if (hasHelp(parsed)) return showHelp(io, DASHBOARD_HELP);
  ...
  if (!input || (input !== "-" && existsSync(input) && statSync(input).isDirectory())) {
    ...
    for (const line of raw.trim().split(/\r\n|\r|\n/u)) {
      if (!line) continue;
      try { records.push(JSON.parse(line) as ExportRecord); }
...
function normalizeShortOutput(args: string[]): string[] {
...
function rejectUnknown(parsed: ParsedArgs, flags: Set<string>, options: Set<string>): void {
  for (const flag of parsed.flags) if (!flags.has(flag) && flag !== "help" && flag !== "h") {
    throw new TodoMdError(`unknown option: --${flag}`);
  }
  for (const option of Object.keys(parsed.options)) if (!options.has(option)) {
    throw new TodoMdError(`unknown option: --${option}`);
  }
}

function showHelp(io: CliIo, help: string): number { io.stdout(help); return 0; }
function usageError(io: CliIo, error: string, help: string): number { io.stderr(error); io.stdout(help); return 1; }
function isRecord(record: ExportRecord | null): record is ExportRecord { return record !== null; }
