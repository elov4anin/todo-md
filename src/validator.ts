...
import {
  ...
  parseSimpleYaml,
  removeBom,
  stripLinkFragmentAndQuery,
} from "./parser.js";

const ASSIGNEE_REQUIRED_STATUSES = ["in_progress", "paused", "blocked", "review", "done"];
const DEFAULT_VALIDATION_CONFIG: TodoMdConfig = { roles: [], agents: [], strict: false };

export function validateFile(
  file: string,
  idIndex: IdIndex,
  config: TodoMdConfig = DEFAULT_VALIDATION_CONFIG,
): ValidationResult {
  const errors: string[] = [];
  ...
  if (!kind) errors.push("file name must start with TASK- or EPIC- and end with .todo.md");
...
export function checkOnBoard(root: string, file: string): ValidationResult {
...
function validateFrontMatter(
  ...
  if (kind === "task") required.push("depends_on", "epic", "branch", "pr");
  else if (kind === "epic") required.push("pr");
  for (const key of required) {
    if (!(key in frontMatter)) errors.push(`missing front matter field \`${key}\``);
  }
  const type = frontMatter.type ?? "";
  if (kind === "epic" && type !== "epic") errors.push("EPIC file must have `type: epic`");
  else if (kind === "task" && !TASK_TYPES.includes(type as never)) {
...
function validateDependencies(
  frontMatter: Record<string, string>, idIndex: IdIndex, errors: string[], warnings: string[],
): void {
  const idPattern = /^(TASK|EPIC)-[A-Za-z0-9][A-Za-z0-9_-]*$/u;
  for (const dependency of (frontMatter.depends_on ?? "").split(",").map((item) => item.trim())) {
    if (dependency === "" || !idPattern.test(dependency)) continue;
    if (!(dependency in idIndex)) errors.push(`\`depends_on\` references unknown ID: ${dependency}`);
...
function validateDate(field: string, value: string, errors: string[]): void {
...
function validateOptionalDate(field: string, value: string, errors: string[]): void {
  if (value !== "") validateDate(field, value, errors);
}

function validateChangeHistory(body: string, errors: string[]): void {
  ...
  if (next?.index !== undefined) rest = rest.slice(0, next.index);
  let headerSeen = false;
  for (const line of rest.split(/\r\n|\r|\n/)) {
    const trimmed = line.trim();
    if (!trimmed.startsWith("|") || /^\|[\s:|-]+\|\s*$/u.test(trimmed)) continue;
    const cells = trimmed.replace(/^\||\|$/gu, "").split("|").map((cell) => cell.trim());
    if (!headerSeen) {
      headerSeen = true;
      continue;
    }
    if (cells[0]) validateDate("change history date", cells[0], errors);
  }
}

function validateEnum(field: string, value: string, allowed: readonly string[], errors: string[]): void {
  if (!allowed.includes(value)) errors.push(`\`${field}\` must be one of: ${allowed.join(", ")}`);
}

function validateOptionalInteger(field: string, value: string, errors: string[]): void {
  if (value !== "" && !/^\d+$/u.test(value)) errors.push(`\`${field}\` must be an integer token count or empty`);
}

function validateTitle(body: string, id: string, errors: string[]): void {
  const match = /^#\s+([^:\n]+):\s+.+$/mu.exec(body);
  if (!match?.[1]) {
...
function validateSections(body: string, kind: TaskKind | null, errors: string[]): void {
  ...
  if (kind === "task") sections.push(["Verification", /^##\s+\d+\.\s+.*Verification/mu]);
  for (const [name, pattern] of sections) if (!pattern.test(body)) errors.push(`missing section: ${name}`);
  if (!/^###\s+.*Must Have/mu.test(body)) errors.push("missing Must Have requirements subsection");
  if (!/^###\s+.*Won[’']?t Have/mu.test(body)) errors.push("missing Won't Have requirements subsection");
}

function validateFolderStatus(file: string, status: string, errors: string[]): void {
  const path = file.replaceAll("\\", "/");
  const folders: Array<[string, string]> = [["done", "done"], ["cancelled", "cancelled"], ["backlog", "backlog"]];
  for (const [folder, expected] of folders) {
    if (path.includes(`/todo/${folder}/`)) {
      if (status !== expected) errors.push(`files in todo/${folder}/ must have \`status: ${expected}\``);
      return;
    }
  }
...
function validateMarkdownLinks(file: string, body: string, errors: string[]): void {
  for (const [, target] of extractMarkdownLinks(body)) {
    ...
    if (!existsSync(candidate)) errors.push(`broken local markdown link \`${target}\``);
  }
}

function validateNoTemplatePlaceholders(content: string, errors: string[]): void {
  ...
  if (known?.[0]) errors.push(`template placeholder found: ${known[0]}`);
  const field = /^\s*[A-Za-z_][A-Za-z0-9_-]*:\s*<[^>\n]+>/mu.exec(content);
  if (field?.[0]) errors.push(`template placeholder found: ${field[0].trim()}`);
  const list = /^\s*(?:[-*]|\d+\.)\s*(?:\[[ xX]\]\s*)?<[^>\n]+>\.?\s*$/mu.exec(content);
  if (list?.[0]) errors.push(`template placeholder found: ${list[0].trim()}`);
  if (/^\s*-\s*\[\s*\]\s*\.\.\.\s*$/mu.test(content)) {
    errors.push("unfinished checklist placeholder found: `- [ ] ...`");
  }
  if (/^\s*-\s*\.\.\.\s*$/mu.test(content)) errors.push("unfinished list placeholder found: `- ...`");
}

function validateAssignee(
  value: string, status: string, config: TodoMdConfig, errors: string[], warnings: string[],
): void {
  if (value.trim() === "") {
    if (!ASSIGNEE_REQUIRED_STATUSES.includes(status)) return;
    actorIssue(errors, warnings, config.strict,
...
function validateActor(
  field: string, value: string, config: TodoMdConfig, errors: string[], warnings: string[],
): void {
  const trimmed = value.trim();
  if (trimmed === "") {
    actorIssue(errors, warnings, config.strict,
      `\`${field}\` must not be empty — expected \`<роль> (<агент>)\`, e.g. \`Бэкендер (codex-cli)\``);
    return;
  }
  const match = /^(.+) \(([a-z0-9][a-z0-9_-]*)\)\s*$/u.exec(trimmed);
  if (!match?.[1] || !match[2]) {
    actorIssue(errors, warnings, config.strict,
      `\`${field}\` must use format \`<роль> (<агент>)\` with a lowercase agent id, e.g. \`Бэкендер (codex-cli)\` — got \`${trimmed}\``);
    return;
  }
  const role = match[1].trim();
  const agent = match[2];
  const agents: readonly string[] = config.agents.length > 0 ? config.agents : AI_AGENTS;
...
function actorIssue(errors: string[], warnings: string[], strict: boolean, message: string): void {
